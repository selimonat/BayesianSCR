...
fit = scr_stan_grid_collect(name='2015-11-24_scr_model1_hierarchicalonset')
#grid engine saving
...
# Evaluate Fit
a = NULL
for (idx in sample.int(100,100)){
  amplist = NULL
  for( o in 1:data_stan$ntrial){
    amplist = c(amplist,S$value[S$Iteration == idx& S$Parameter == paste0('amp_per_onset.1.',o,'.')])
  }
  scr = scr_model(x=data_stan$x,onsets = data_stan$onset,latency = rep(S$value[S$Iteration == idx & S$Parameter=='latency.1.'],data_stan$ntrial),
                  amp = amplist,
                  tau1 = S$value[S$Iteration == idx & S$Parameter=='tau1'],
                  tau2 = S$value[S$Iteration == idx & S$Parameter=='tau2'])
